...
if "valve_status" not in st.session_state:
    st.session_state.valve_status = {}  # Track which valves are shut
...
def create_pipe_layer(gdf, valve_shut_ids=None, highlight_disconnected=False):
    ...
    for _, row in gdf.iterrows():
        ...
        if highlight_disconnected and valve_shut_ids:
            color = [150, 150, 150]  # Disconnected: grey
        features.append({
            "path": coords,
...
# --- PAGE 2: Interactive Map Dashboard ---
def show_dashboard():
    ...
    if leak_gdf is not None and "DateRepor" in leak_gdf.columns:
        ...
        years = leak_gdf["year"].dropna().unique()
        if len(years) > 0:
            year = st.sidebar.slider("🕓 Leak Year", int(years.min()), int(years.max()), int(years.max()))
    ...
    # Valve simulation logic (popup simulation can be added later)
    tooltip = {
